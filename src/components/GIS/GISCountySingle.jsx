...
export default function GISCountySingle(props) {
  ...
  function fetchAll(datalayer, url, color) {
    ...
    if (filter.length === 0) {
      setLoading(true);
      fetch(`/api/${url}/geojson/${props.county}/${start}/${end}`)
        .then((res) => {
  ...
  function fetchBeneficiaries() {
    setLoading(true);
    fetch(`/api/waterproviders/bycounty/${props.county}`)
      .then((res) => {
        if (res.ok) return res.json();
        else throw Error("");
      })
  ...
  function triangleIcon() {
    ...
    // Define the triangle shape on the canvas
    context.beginPath();
  ...
  useEffect(() => {
    if (map) {
      setLoading(true);
      fetch(`/api/counties/gis/boundary/${props.county}`)
        .then((res) => {
          if (res.ok) return res.json();
          else throw Error("");
        })
    ...
    draw.on("drawend", (event) => {
      ...
      setTimeout(() => {
        ...
        html2canvas(mapElement.current).then(function (canvas) {
          ...
          setDataUrl(null);
          setMapExport(false);
          window.open(`/counties/${props.county}/export`, "target:_blank");
        });
      }, 3000);
    });
  }
